...
def bottleneck(filters, *args, **kwargs):
    return Sequential(
        [conv_block(filters // 4, (1, 1), padding="same"), conv_block(filters, (3, 3), padding="same"),],
        *args,
        **kwargs,
    )
...
def FeaturePyramidNet(
    ...
    **kwargs,
    ...
    output_shapes = (
        pd.DataFrame(
            [
                layer.input_shape[0] if isinstance(layer.input_shape, list) else layer.output_shape
    ...
    for output_shape in output_shapes.iloc[:feature_maps].itertuples():
        ...
        if outputs:
            pyramid_input = up_sampling_block(output_shape.channels, name=f"up_sampling_{output_shape.channels}")(
                outputs[-1]
            )
    ...
    if classes is not None:
        ...
        outputs = Concatenate(axis=1)(
            [
                Lambda(
                    lambda output, index_=index: (
                        tf.concat(
                            [output, tf.expand_dims(tf.ones(tf.shape(output)[:2], dtype=output.dtype) * index_, -1)],
                            axis=-1,
                        )
                    )
                )(outputs[index])
